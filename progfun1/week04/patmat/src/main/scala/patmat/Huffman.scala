...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Fork(l, r,  ls, w) => w
    case Leaf(c, w) => w
  }
  
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(l, r, ls, w) => ls
    case Leaf(c, w) => c :: Nil
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortWith((one, other) => one._2 < other._2).map(x => Leaf(x._1, x._2))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case l :: r :: xs => (makeCodeTree(l, r) :: xs).sortWith((one, other) => weight(one) < weight(other))
    case _ => trees
  }
